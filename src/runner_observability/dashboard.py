...
def _runner_view(store: Store, runner: Mapping[str, Any], window: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if (
        ...
        if job_row is not None:
            current_job = _job_view(job_row, _job_history(window, job_row))
    return {
        "runner_id": runner["runner_id"],
        "alias": _alias(runner["runner_id"]),
        "liveness": runner["liveness"],
        "activity": runner["activity"],
        "offline_reason": runner["offline_reason"],
        "last_received_at": runner["last_received_at"],
...
def _job_view(job_row: Mapping[str, Any], job_history: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "last_received_at": job_row["last_received_at"],
        ...
        "timeline": [_timeline_entry(item) for item in job_history],
    }
...
def _event_feed(window: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ordered = sorted(window, key=_received_at_key, reverse=True)[:EVENT_FEED_LIMIT]
    return [
        {
            "event_type": item["event_type"],
            "runner_id": item["runner_id"],
            "runner_alias": _alias(item["runner_id"]),
            "received_at": item["received_at"],
            "occurred_at": item["occurred_at"],
